...
class _PermissionsScreenState extends State<PermissionsScreen> {
  ...
  Future<void> _load({String? status, bool? myOnly}) async {
    ...
    setState(() {
      ...
      _error = null;
      _filter = status;
      _myOnly = resolvedMyOnly;
    });
    try {
      final myId = resolvedMyOnly ? (AuthService.user?['id'] as int?) : null;
      final data =
          await ApiService.getPermissionRequests(status: status, userId: myId);
      final raw = data['data'];
      setState(() {
        ...
        children: [
          ...
          if (AuthService.isRH) ...[
            Padding(
              ...
              child: FilterChip(
                label:
                    const Text('Mes demandes', style: TextStyle(fontSize: 12)),
                selected: _myOnly,
          ...
          ...List.generate(
              ...
              (i) => Padding(
                    ...
                    child: FilterChip(
                      ...
                      onSelected: (_) =>
                          _load(status: filters[i], myOnly: false),
  ...
  Future<void> _showDetail(
      Map<String, dynamic> r, bool isMySubordinate, bool isDGRole) async {
      ...
      builder: (_) => DraggableScrollableSheet(
        ...
        builder: (_, ctrl) => SingleChildScrollView(
          ...
          child:
            ...
            _detailRow(
                'Type', _typeLabels[full['type']] ?? full['type'] ?? '—'),
            if (isConge && full['conge_subtype'] != null)
              _detailRow(
                  'Précision',
                  _subtypeLabels[full['conge_subtype']] ??
                      full['conge_subtype']),
            if (isConge) ...[
              ...
              _detailRow('Période',
                  _periodeLabels[full['periode']] ?? full['periode'] ?? '—'),
              _detailRow('Durée',
                  '${full['duree_valeur']} ${full['duree_unite'] == 'heure' ? 'heure(s)' : 'jour(s)'}'),
            ...
            if (full['manager_decision'] != null) ...[
              ...
              _detailRow(
                  'Décision',
                  full['manager_decision'] == 'approved'
                      ? '✅ Validé'
                      : '❌ Refusé'),
            ...
            if (full['dg_decision'] != null) ...[
              ...
              _detailRow(
                  'Décision',
                  full['dg_decision'] == 'approved'
                      ? '✅ Accordé'
                      : full['dg_decision'] == 'refused'
                          ? '❌ Refusé'
                          : '✏️ Modification'),
            ...
            if (isMySubordinate && status == 'pending_manager') ...[
              ...
              Row(children: [
                Expanded(
                  ...
                  onPressed: () {
                    Navigator.pop(context);
                    _managerDecide(full['id'] as int, 'approved');
                  },
                ...
                Expanded(
                  ...
                  onPressed: () {
                    Navigator.pop(context);
                    _managerDecide(full['id'] as int, 'refused');
                  },
            ...
            if (isDGRole && status == 'pending_dg') ...[
              ...
              Row(children: [
                Expanded(
                  ...
                  onPressed: () {
                    Navigator.pop(context);
                    _dgDecide(full['id'] as int, 'approved');
                  },
                ...
                Expanded(
                  ...
                  onPressed: () {
                    Navigator.pop(context);
                    _dgDecide(full['id'] as int, 'refused');
                  },
                  child: const Text('✕ Refuser'),
                )),
              ]),
              ...
              SizedBox(
                  ...
                  child: OutlinedButton(
                    onPressed: () {
                      Navigator.pop(context);
                      _dgDecide(full['id'] as int, 'modification_requested');
                    },
  ...
  Future<void> _managerDecide(int id, String decision) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Décision enregistrée ✅'),
            backgroundColor: AppTheme.success));
      }
      _load(status: _filter);
    } catch (e) {
...
class _PermissionForm extends StatefulWidget {
  ...
  const _PermissionForm(
      {this.defaults, this.resubmitId, this.editId, required this.onSubmitted});
...
class _PermissionFormState extends State<_PermissionForm> {
  ...
  void initState() {
    ...
    if (d != null) {
      ...
      if (d['date_debut'] != null)
        try {
          _debut = DateTime.parse(d['date_debut']);
        } catch (_) {}
      if (d['date_fin'] != null)
        try {
          _fin = DateTime.parse(d['date_fin']);
        } catch (_) {}
      if (d['date_depart'] != null)
        try {
          _depart = DateTime.parse(d['date_depart']);
        } catch (_) {}
      if (d['date_reprise'] != null)
        try {
          _reprise = DateTime.parse(d['date_reprise']);
        } catch (_) {}
  ...
  Future<void> _submit() async {
    ...
    if (isConge) {
      if (_debut == null || _fin == null) {
        setState(
            () => _error = 'Les dates de début et de fin sont obligatoires.');
        return;
      }
    } else {
      if (_depart == null || _reprise == null) {
        setState(() =>
            _error = 'Les dates de départ et de reprise sont obligatoires.');
        return;
      }
    }
    setState(() {
    ...
    if (isConge) {
      if (_subtype != null && _subtype!.isNotEmpty)
        payload['conge_subtype'] = _subtype;
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          DropdownButtonFormField<String>(
            ...
            items: const [
              DropdownMenuItem(
                  ...
                  child: Text('Congé exceptionnel')),
              DropdownMenuItem(
                  value: 'autorisation_absence',
                  child: Text("Autorisation d'absence")),
            ],
          ...
          if (isConge) ...[
            Row(children: [
              Expanded(
                  child: _datePick('Date de début *', _debut,
                      (d) => setState(() => _debut = d))),
              const SizedBox(width: 12),
              Expanded(
                  child: _datePick(
                      'Date de fin *', _fin, (d) => setState(() => _fin = d))),
            ]),
          ] else ...[
            DropdownButtonFormField<String>(
              value: _periode,
              ...
              items: const [
                DropdownMenuItem(
                    value: 'journee', child: Text('Journée entière')),
                DropdownMenuItem(value: 'matin', child: Text('Matin')),
                DropdownMenuItem(
                    value: 'apres_midi', child: Text('Après-midi')),
              ],
            ...
            Row(children: [
              Expanded(
                ...
                child: TextFormField(
                  ...
                  decoration:
                      const InputDecoration(labelText: "Durée d'absence *"),
                  keyboardType:
            ...
            Row(children: [
              Expanded(
                  child: _datePick('Date de départ *', _depart,
                      (d) => setState(() => _depart = d))),
              const SizedBox(width: 12),
              Expanded(
                  child: _datePick('Date de reprise *', _reprise,
