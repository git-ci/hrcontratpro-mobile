                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) => _ContractCard(
                          ...
                          onTap: () => _showDetail(
                              _contracts[i] as Map<String, dynamic>),
                        ),
                      ),
                    ),
    );
...
class _ContractCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cfg = _contractStatuses[status] ??
    ...
    final salary =
        c['salary'] != null ? '${c['salary']} ${c['currency'] ?? ''}' : '—';
    ...
    return Card(
      child: ListTile(
        ...
        leading: Container(
          ...
          child: Center(
              child: Text(type.substring(0, type.length.clamp(0, 3)),
                  style: TextStyle(
                      fontSize: 11,
...
class _ContractDetail extends StatefulWidget {
  ...
  @override
  State<_ContractDetail> createState() => _ContractDetailState();
}
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('PDF joint avec succès'),
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Text(
                          _pdfLoading
                              ? 'Ouverture en cours…'
                              : 'Contrat PDF joint — Appuyer pour ouvrir',
                          style: const TextStyle(
                              color: AppTheme.success,
...
class _RequestFormState extends State<_RequestForm> {
  ...
  Future<void> _submit() async {
    if (_reason.text.isEmpty) {
      setState(() => _error = 'La raison est obligatoire.');
      return;
    }
    setState(() {
      _loading = true;
    ...
    try {
      await ApiService.createRequest({
        ...
        if (_salary.text.isNotEmpty)
          'proposed_salary': double.tryParse(_salary.text),
        if (_endDate != null)
          'proposed_end_date': _endDate!.toIso8601String().substring(0, 10),
        if (_type == 'type_change') 'proposed_contract_type': _newType,
      });
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Demande soumise !'),
            backgroundColor: AppTheme.success));
        Navigator.pop(context);
      }
    } catch (e) {
      ...
      child: SingleChildScrollView(
        ...
        if (_type == 'salary_change')
          TextField(
              ...
              keyboardType: TextInputType.number,
              decoration:
                  const InputDecoration(labelText: 'Nouveau salaire proposé')),
        if (_type == 'renewal')
          OutlinedButton.icon(
            icon: const Icon(Icons.event, size: 16),
        ...
        TextField(
            ...
            maxLines: 3,
            decoration:
                const InputDecoration(labelText: 'Raison / Justification *')),
        if (_error != null) ...[
          const SizedBox(height: 8),
          Text(_error!,
              style: const TextStyle(color: AppTheme.danger, fontSize: 12)),
...
class _EditContractForm extends StatefulWidget {
  ...
  @override
  State<_EditContractForm> createState() => _EditContractFormState();
}
...
class _EditContractFormState extends State<_EditContractForm> {
  ...
  void initState() {
    ...
    if (c['end_date'] != null) {
      try {
        _end = DateTime.parse(c['end_date']);
      } catch (_) {}
    }
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Contrat mis à jour ✅'),
            backgroundColor: AppTheme.success));
        Navigator.pop(context);
      }
    } catch (e) {
